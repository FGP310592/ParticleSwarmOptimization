        ...
        for IndexPar = 1:1:NumPar
            ...
            for IndexObjFun = 1:1:LengthObjFun
                if(Sett.ObjFunType(IndexObjFun) == "max")
                    ObjFun(IndexPar,LengthObjFun) = -ObjFun(IndexPar,LengthObjFun);
                end
            end
        end
        % Determine Xpbest
        ...
        if rem(IndexIter,Sett.NumIterBtwnZeroTests) == 0 && IndexIter ~= 1
            ...
            for IndexX = 1:1:LengthX
                ...
                for IndexPar = 1:1:NumPar
                    ...
                    for IndexObjFun = 1:1:LengthObjFun
                        if(Sett.ObjFunType(IndexObjFun) == "max")
                            ObjFunpbestguest(IndexPar,LengthObjFun) = -ObjFunpbestguest(IndexPar,LengthObjFun);
                        end
                    end
                end
...
for IndexObjFun = 1:1:LengthObjFun
    if(Sett.ObjFunType(IndexObjFun) == "max")
        ObjFunfbest(LengthObjFun) = -ObjFunfbest(LengthObjFun);
    end
end
% Plots - Pt.2
if(Sett.FlagPlots == true)
